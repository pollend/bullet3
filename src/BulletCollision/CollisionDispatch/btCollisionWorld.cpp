	...
	else
	{
		if (collisionShape->isConcave())
		{
			...
			if (collisionShape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE)
			{
				...
				BridgeTriangleRaycastCallback rcb(rayFromLocal, rayToLocal, &resultCallback, collisionObjectWrap->getCollisionObject(), triangleMesh, colObjWorldTransform, collisionObjectWrap->getVoxelInfo());
			...
			else if (collisionShape->getShapeType() == SCALED_TRIANGLE_MESH_SHAPE_PROXYTYPE)
			{
				...
				BridgeTriangleRaycastCallback rcb(rayFromLocalScaled, rayToLocalScaled, &resultCallback, collisionObjectWrap->getCollisionObject(), triangleMesh, colObjWorldTransform, collisionObjectWrap->getVoxelInfo());
				rcb.m_hitFraction = resultCallback.m_closestHitFraction;
			...
			else if (((resultCallback.m_flags & btTriangleRaycastCallback::kF_DisableHeightfieldAccelerator) == 0) && collisionShape->getShapeType() == TERRAIN_SHAPE_PROXYTYPE)
			{
				...
				BridgeTriangleRaycastCallback rcb(rayFromLocal, rayToLocal, &resultCallback, collisionObjectWrap->getCollisionObject(), heightField, colObjWorldTransform, collisionObjectWrap->getVoxelInfo());
				rcb.m_hitFraction = resultCallback.m_closestHitFraction;
			...
			else
			{
				...
				BridgeTriangleRaycastCallback rcb(rayFromLocal, rayToLocal, &resultCallback, collisionObjectWrap->getCollisionObject(), concaveShape, colObjWorldTransform,collisionObjectWrap->getVoxelInfo());
				rcb.m_hitFraction = resultCallback.m_closestHitFraction;
						...
						for (int y = min[1]; y <= max[1]; ++y) {
							for (int z = min[2]; z <= max[2]; ++z) {
								...
								if (childInfo.m_tracable) {
									...
									btCollisionObjectWrapper tmpOb(colObjWrap, childInfo.m_collisionShape, colObjWrap->getCollisionObject(), voxelTransform, -1, -1,colObjWrap->getVoxelInfo());
